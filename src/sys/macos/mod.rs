...
impl Watcher {
    ...
    pub fn iter_windows(&self) -> impl Iterator<Item = Result<Window, WindowError>> + '_ {
        ...
        self.apps.iter().flat_map(|app| {
            app.windows()
                .into_iter()
                .flat_map(|windows| windows.into_iter().map(Ok))
        })
    }
...
pub fn request_trust() -> bool {
    let options = unsafe {
        CFDictionaryCreate(
            kCFAllocatorDefault,
            &mut [kAXTrustedCheckOptionPrompt].as_ptr() as *mut _ as *mut _,
            &mut [kCFBooleanTrue].as_ptr() as *mut _ as *mut _,
            1,
            ptr::null(),
            ptr::null(),
        )
...
impl WindowError {
    ...
    pub(self) fn from_ax_error(code: i32) -> WindowError {
        match code {
            ffi::kAXErrorAPIDisabled => WindowError::ApiDisabled,
